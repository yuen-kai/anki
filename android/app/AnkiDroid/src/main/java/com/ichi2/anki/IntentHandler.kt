...
import com.ichi2.anki.pages.SpeedrunFragment
...
class IntentHandler : AbstractIntentHandler() {
    ...
    private fun launchDeckPickerIfNoOtherTasks(reloadIntent: Intent) {
        // The Speedrun SvelteKit shell is the app's primary UI, so a plain
        // launcher intent opens it instead of the stock deck list. The whole
        // shell (decks, overview, authoring, dashboard, study) runs client-side
        // in one WebView. (reloadIntent -> DeckPicker is still used by the
        // import/sync/review flows that route through here.)
        Timber.i("Launching Speedrun home")
        // A plain launch of the generic single-fragment host: the launcher's
        // MAIN/LAUNCHER + CLEAR_TASK flags belong to DeckPicker (the app's main
        // activity) and clear the task out from under a generic host.
        startActivity(SpeedrunFragment.getIntent(this))
        finish()
    }
