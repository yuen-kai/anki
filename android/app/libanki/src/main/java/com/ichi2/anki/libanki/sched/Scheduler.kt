...
// Bespoke Speedrun study screen (speedrun-review): the shared Rust engine's
// study RPCs, so the AnkiDroid webview drives the same backend as desktop.
fun Collection.speedrunStudyStateRaw(input: ByteArray): ByteArray = backend.speedrunStudyStateRaw(input = input)

fun Collection.speedrunNextCardRaw(input: ByteArray): ByteArray = backend.speedrunNextCardRaw(input = input)

fun Collection.speedrunAnswerCardRaw(input: ByteArray): ByteArray = backend.speedrunAnswerCardRaw(input = input)

fun Collection.speedrunRecordLearnedRaw(input: ByteArray): ByteArray = backend.speedrunRecordLearnedRaw(input = input)

fun Collection.speedrunStudyHierarchyRaw(input: ByteArray): ByteArray = backend.speedrunStudyHierarchyRaw(input = input)

// Speedrun deck/authoring store CRUD (the deck/hierarchy/study screens), also
// shared with desktop via SchedulerService, so the AnkiDroid shell drives the
// same authoring backend.
fun Collection.speedrunListDecksRaw(input: ByteArray): ByteArray = backend.speedrunListDecksRaw(input = input)

fun Collection.speedrunGetHierarchyRaw(input: ByteArray): ByteArray = backend.speedrunGetHierarchyRaw(input = input)

fun Collection.speedrunSaveHierarchyRaw(input: ByteArray): ByteArray = backend.speedrunSaveHierarchyRaw(input = input)

fun Collection.speedrunDeleteDeckRaw(input: ByteArray): ByteArray = backend.speedrunDeleteDeckRaw(input = input)
...
// with desktop.
fun Collection.getMemoryScoreRaw(input: ByteArray): ByteArray = backend.getMemoryScoreRaw(input = input)

fun Collection.getPerformanceScoreRaw(input: ByteArray): ByteArray = backend.getPerformanceScoreRaw(input = input)

fun Collection.getReadinessScoreRaw(input: ByteArray): ByteArray = backend.getReadinessScoreRaw(input = input)

fun Collection.getSpeedrunScoreBreakdownRaw(input: ByteArray): ByteArray = backend.getSpeedrunScoreBreakdownRaw(input = input)
