...
// ../bridge (when `local_backend=true` in local.properties) or as
// the published library from the version catalog.

// Paths within the `bridge` checkout (android/bridge), sibling of android/app.
...
private fun Project.addBackendArtifact(
    ...
    if (useLocalBackend) {
        // ../bridge (the rsdroid engine-bridge, sibling of android/app in the monorepo)
        val backendCheckout = File(rootProject.projectDir.parentFile, "bridge")
