...
> Note: this library is now vendored into the Speedrun monorepo at `android/bridge`.
> It is no longer a standalone repo or a git submodule, and the shared engine lives at
> the repo root (there is no `./anki` symlink). Build it through the monorepo: see the
> top-level [`README.md`](../../README.md) and
> [`ProjectDocs/CODEBASE.md`](../../ProjectDocs/CODEBASE.md). The standalone, two-repo
...
_Note:_ n2 receives occasional mandatory updates. If you see build errors, you may need to re-run this command and re-try the build
