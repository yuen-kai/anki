...
fn build_web_artifacts() -> Result<()> {
    ...
    copy_dir_all("../../out/sveltekit", artifacts_dir.join("sveltekit"))?;
    ...
    let mut ts_code = vec![];
    ts_code.append(&mut ts_all_scripts(Path::new("../../ts")).unwrap());
    // Also include scripts inside of Svelte files.
    ts_code.append(&mut svelte_all_scripts(Path::new("../../ts")).unwrap());
    ...
    copy_file(
        "../../out/ts/reviewer/reviewer_extras_bundle.js",
        artifacts_dir.join("js/reviewer_extras_bundle.js"),
    )?;
    copy_file(
        "../../out/ts/reviewer/reviewer_extras.css",
        artifacts_dir.join("css/reviewer_extras.css"),
    )?;
    copy_file(
        "../../out/ts/reviewer/reviewer.js",
        artifacts_dir.join("js/reviewer.js"),
    )?;
    copy_file(
        "../../out/ts/reviewer/reviewer.css",
        artifacts_dir.join("css/reviewer.css"),
    )?;

    copy_dir_all(
        "../../out/qt/_aqt/data/web/js/vendor/mathjax",
        artifacts_dir.join("js/vendor/mathjax"),
    )?;
    copy_file(
        "../../out/ts/mathjax/mathjax.js",
    ...
    copy_file(
        "../../out/ts/lib/sass/_root-vars.css",
...
fn build_robolectric_jni() -> Result<()> {
    ...
    let target_root = Utf8Path::new("../../out/rust");
    let file_in_target =
