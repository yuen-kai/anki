...
- **Post-phase (three honest scores, [D33](decisions.md#d33)):** all three scores are now built and shown separately (the brief's §4). **Memory** (FSRS recall) was already live; **Performance** (exam-weight-weighted accuracy over `SpeedrunApplication` attempts) and **Readiness** (a 472-528 projection with a coverage-widened range) are now live too, each with the shared evidence envelope + give-up rule. Engine: 11 rust tests; e2e: 4 python; dashboard: 5 vitest; all green. Only the paraphrase-test validation (Friday) + calibration (Sunday) stay deferred, stated in each score's reasons ([B030](backlog.md#b030)). (The two build subagents were billing-blocked, so this was built directly.)
